...
#ifndef LUX_Run3_hh
#define LUX_Run3_hh 1

#include "VDetector.hh"

using namespace std;

class LUX_Run3: public VDetector {
 public:
  LUX_Run3() {
    cerr << "*** Detector definition message ***" << endl;
		cerr << "You are currently using the default LUX Run 3 (WS) detector settings."
         << endl
         << endl;
  ...
  virtual void Initialization() {
    ...
    g1 = 0.1167;  // phd per S1 phot at dtCntr (not phe). Divide out 2-PE effect
    ...
    g1_gas = 0.0965;  // phd per S2 photon in gas, used to get SE size
    s2Fano = 3.7;  // Fano-like fudge factor for SE width
    s2_thr = (150.*1.173)/0.915;  // the S2 threshold in phe or PE, *not* phd. Affects NR most
    E_gas = 6.14;    // field in kV/cm between liquid/gas border and anode
    ...
    radius = 200.;  // millimeters (fiducial rad)
    radmax = 200.;  // actual physical geo. limit

    TopDrift = 544.2;  // mm not cm or us (but, this *is* where dt=0)
  ...
  virtual vector<double> SinglePEWaveForm(double area, double t0) {
	...
	virtual void SetParameters_CH3T() {
		// Primary Scintillation (S1) parameters
		set_g1(0.116);
		set_sPEres(0.37);
