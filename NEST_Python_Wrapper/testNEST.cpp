 ...
 * version 2.4 - 20180712
 *         - move v_table out of main function to wrapper so that i only compute once when loop main function
 *         - add function to input and output for qy ly nuissan parameters
 *         - add qy, ly output
 *         - now also need to modify NEST.cpp and NEST.hh in order to output qy and ly
 ...
 * Note that now NEST.hh and NEST.cpp also need to be modified in order to add qy and ly output
 ...
 * In the output
    ...
    boost::python::list ly;
    boost::python::list qy;
    boost::python::list deposite_energy_keV;
...
class nest_py_wrapper{
  protected:
    ...
    char** argv;
    nestOP nest_op;
    double nuisParam_ly = 1, nuisParam_qy = 1;
    bool print_result = true;
    vector<double> vTable;
    vector<double> get_vTable();
    int testNEST(int argc, char** argv, double nuisParam_ly, double nuisParam_qy, const vector<double>& vTable, bool print_result,
		 ...
		 vector<double>& s2_raw_area_phe, vector<double>& s2c_area_phe, vector<double>& s2_raw_area_phd, vector<double>& s2c_area_phd, vector<double>& ly, vector<double>& qy,
		 vector<double>& deposite_energy_keV, vector<double>& pos_x_mm, vector<double>& pos_y_mm, vector<double>& pos_z_mm, vector<double>& field_V_per_cm);
  public:
    void disable_console_print(){print_result = false;}
    void enable_console_print(){print_result = true;}
    void set_nuisParam_ly(double nuisparam_ly){nuisParam_ly = nuisparam_ly;}
    double get_nuisParam_ly(){return nuisParam_ly;}
    void set_nuisParam_qy(double nuisparam_qy){nuisParam_qy = nuisparam_qy;}
    double get_nuisParam_qy(){return nuisParam_qy;}
};

vector<double> nest_py_wrapper::get_vTable(){
  double rho;
  vector<double> vTable;
  LUX_Run4* detector = new LUX_Run4();
  NEST::NESTcalc n(detector);
  rho = n.SetDensity ( detector->get_T_Kelvin(), detector->get_p_bar() ); //cout.precision(12);
...
nestOP nest_py_wrapper_mass::testNEST(int numEvt){
  ...
  vector<double> ly;
  vector<double> qy;
  vector<double> deposite_energy_keV;
  ...
  for(int i = 0; i < numEvt; i++){
    ...
    nest_py_wrapper::testNEST(argc, argv, nuisParam_ly, nuisParam_qy, vTable, print_result, nph, ne, s1_n_hits, s1_n_phe,
                              s1_raw_area_phe, s1c_area_phe, s1_raw_area_phd, s1c_area_phd, s1_raw_spike, s1c_spike,
                              s2_ne_extract, s2_n_ph, s2_n_hits, s2_n_phe,
                              s2_raw_area_phe, s2c_area_phe, s2_raw_area_phd, s2c_area_phd, ly, qy,
			      deposite_energy_keV, pos_x_mm, pos_y_mm, pos_z_mm, field_V_per_cm);
  }
  free(argv);
  reset_input();
  ...
  nest_op.ly = (std_vector_to_py_list<double>(ly));
  nest_op.qy = (std_vector_to_py_list<double>(qy));  
  nest_op.deposite_energy_keV = (std_vector_to_py_list<double>(deposite_energy_keV));
...
int nest_py_wrapper::testNEST(int argc, char** argv, double nuisParam_ly, double nuisParam_qy, const vector<double>& vTable, bool verbosity,
	     vector<int>& nph, vector<int>& ne, vector<int>& s1_n_hits, vector<int>& s1_n_phe,
	     vector<double>& s1_raw_area_phe, vector<double>& s1c_area_phe, vector<double>& s1_raw_area_phd, vector<double>& s1c_area_phd, vector<double>& s1_raw_spike, vector<double>& s1c_spike,
	     vector<int>& s2_ne_extract, vector<double>& s2_n_ph, vector<double>& s2_n_hits, vector<double>& s2_n_phe,
			      vector<double>& s2_raw_area_phe, vector<double>& s2c_area_phe, vector<double>& s2_raw_area_phd, vector<double>& s2c_area_phd, vector<double>& ly, vector<double>& qy,
	...
	LUX_Run4* detector = new LUX_Run4();   // make sure detector class, i.e LUX_Run4 or LUX_Run3 matches with that in get_vTable() function (simpy search for get_vTable() and you will know)
  ...
  //    NuisParam = {1., 1.};  // scaling factors, for now just for NR Ly & Qy.
                             // But must initialize!
  vector<double> signal1,signal2,signalE, NuisParam={nuisParam_ly,nuisParam_qy};
  ...
  Z_NEW:
    ...
    if (j == 0 && vD_middle == 0.) {
      if (inField == -1.) {
        ...
        // this part to be modified for python wrapper --->
        // in the python wrapper, comment out vTable, now vTable is an passed in argument for speeding up  
    ...
    if (verbosity) {  // fabs(scint[7]) > PHE_MIN && fabs(scint2[7]) > PHE_MIN ) { //if
    ...
    ly.push_back(yields.Ly);
    qy.push_back(yields.Qy);
    deposite_energy_keV.push_back(keV);
...
BOOST_PYTHON_MODULE(nest_py_interface)
{
    ...
    class_<nest_py_wrapper>("nest_py_wrapper")
      .def("disable_console_print", &nest_py_wrapper::disable_console_print)
      .def("enable_console_print", &nest_py_wrapper::enable_console_print)
      .def("set_nuisParam_ly", &nest_py_wrapper::set_nuisParam_ly)
      .def("set_nuisParam_qy", &nest_py_wrapper::set_nuisParam_qy)
      .def("get_nuisParam_ly", &nest_py_wrapper::get_nuisParam_ly)
      .def("get_nuisParam_qy", &nest_py_wrapper::get_nuisParam_qy)
      ;
    ...
    .def_readonly("ly",&nestOP::ly)
    .def_readonly("qy",&nestOP::qy)
    .def_readonly("deposite_energy_keV",&nestOP::deposite_energy_keV)
