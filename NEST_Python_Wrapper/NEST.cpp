...
YieldResult NESTcalc::GetYields(INTERACTION_TYPE species, double energy,
  ...
  switch (species) {
              ...
              // (TestSpectra)
    {
      ...
      double ThomasImel =
          0.0522 * pow(dfield, -0.0694) * pow(density / DENSITY, 0.3);
    ...
    default:  // beta, CH3T
    {
      ...
      double QyThesJon = (25.26789006+(4.36553313-25.26789006)/(1+pow(dfield/115.54855366,1.74807285)))
        +(77.2931084-(25.26789006+(4.36553313-25.26789006)/(1.+pow(dfield/115.54855366,1.74807285))))/
        ...
        +42.83185544+(0.-42.83185544)/pow((1.+pow(energy/(7.02921301+(98.27936794-7.02921301)/
	(1.+pow(dfield/256.48156448,1.29119251))),4.285781736)),(1.08273937e-01+(8.66087199e-02-1.08273937e-01)/(1+pow(dfield/1.39260460e+02,-4.65763592e+00))));
      double QyMattPub = QyLvlmedE + (QyLvllowE - QyLvlmedE) / pow(1. + 1.304 * pow(energy, 2.1393), 0.35535) + QyLvlhighE / (1. + DokeBirks * pow(energy, LET_power));
      double weight = ( energy / 156. ) * ( 81. - 0.9 * dfield + 0.0025 * dfield * dfield );
      if ( weight > 1. ) weight = 1.;
      if ( weight < 0. ) weight = 0.;
      double Qy = QyThesJon;//( 1. - weight ) * QyMattPub + weight * QyThesJon;
...
// this part to be added for python wrapper --->
  result.Ly = Ly*NuisParam[0];
  result.Qy = Qy*NuisParam[1];
// this part to be added for python wrapper <---
...
vector<double> NESTcalc::GetS1(QuantaResult quanta, double truthPos[3],
  ...
  if (spike < 0) spike = 0;
  // added so that there is no XY correction to match with data - DQ 20180816
  posDepSm = fdetector->FitS1(0, 0, smearPos[2])/fdetector->FitS1(0., 0., dz_center);
  // added so that there is no XY correction to match with data - DQ 20180816
  ...
  if (spike >= 1. && spikeC > 0.) {
    ...
    //		newSpike = GetSpikeRun4(spike, smearPos[0], smearPos[1], smearPos[2], dt, 
...
vector<double> NESTcalc::GetSpike(int Nph, double dx, double dy, double dz, double dt,
...
vector<double> NESTcalc::GetSpikeRun4 ( double spike, double dx, double dy, double dz, double dt,
  double driftSpeed, double dS_mid, INTERACTION_TYPE species ) {
	...
	numPHETop = (double) BinomFluct(spike, topArrayProbabilityCoeffs[0]*pow(dt,2.) + topArrayProbabilityCoeffs[1]*dt + topArrayProbabilityCoeffs[2]);
	numPHEBot = (double) spike - numPHETop;
